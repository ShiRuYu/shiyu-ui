...
├─ store/         认证及业务状态
├─ utils/         应用工具
└─ features/      领域 Feature Slice 页面与公共入口
```
...
- 后端业务路由：登录后调用 `/api/iam/menus/all` 获取，由 Vben access 模块动态生成。

各领域 `features/<domain>/index.ts` 导出页面组件。后端菜单 `component` 使用 `feature:knowledge.spaces` 等语义键，由菜单契约映射到对应 feature 公共入口。
...
- 每个节点必须有非空 `path`。
- `children` 必须是数组。
- 字符串组件必须是 `BasicLayout`、`IFrameView` 或已注册的 `feature:<domain>.<page>` 语义键。

契约失败时进入 `/menu-load-error`，用户可重试，不会静默渲染不完整菜单。

新增页面的完整链路：

1. 在对应 `features/<domain>` 下创建页面和组件。
2. 在后端 `AUTH_MENU` 新增/迁移菜单，确保 `name` 全局唯一。
3. `component` 写 `feature:<domain>.<page>` 语义键。
4. 为角色分配菜单和权限码。
5. 重新登录或刷新访问上下文，验证桌面和移动端。

## 5. 请求层

`src/shared/api/request.ts` 创建两个客户端：
...
- 在允许刷新时调用 `/api/iam/auth/refresh`。
- 刷新失败或未登录时清空会话并跳转登录页。
- 统一显示服务端 `error`/`message`，避免过期会话重复弹错。

### 5.1 API 前缀

开发和生产环境的 `VITE_GLOB_API_URL` 都默认 `/api`。开发服务器保留领域 API 前缀：

```ts
'/api' -> target VITE_PROXY_TARGET || 'http://localhost:9000'
rewrite: preserve /^\/api/
```

后端 Controller 使用 `/api/{domain}` 统一前缀。生产网关必须保留相同转发语义。
...
- `/api/iam/users/detail` 用户信息。
- `/api/iam/auth/codes` 权限码。
- `/api/iam/menus/all` 菜单。
- `/api/iam/auth/tenants` 可用租户。
...
| 目录 | 职责 |
| --- | --- |
| `app/pages/workbench` | 平台概览与分析工作台；浏览器路由为 `/workbench` |
| `features/agent` | Agent 定义、版本、图编排、意图和执行 |
| `features/conversation` | Chat 会话、消息树、Prompt 和生成流 |
| `features/model` | 模型平台、模型目录和默认模型 |
| `features/knowledge` | 工作台、空间、资产、文档、图谱、检索、评测、索引和运维 |
| `features/education` | 课程、知识点、学习计划、练习、考试、复习、资源、AI 家教和教育后台 |
| `features/governance` | 用量、配额、运行观测和治理报告 |
| `features/iam` | 用户、租户、角色、菜单、权限码、字典和文件 |
| `features/memory` | 记忆事件、关系和索引操作入口 |
| `features/tooling` | 插件市场、工具注册和 MCP 能力 |
...
- 主布局和个人设置页在租户/角色切换后会重置动态路由、清空访问状态并触发 `/api/iam/menus/all` 重载；菜单刷新逻辑符合后端上下文切换设计。
- 自动 Token 续期使用 `/api/iam/auth/refresh`，前端刷新请求只在 body 发送旧 Token，由后端控制器完成轮换校验。
- 后端 `/ws/usage` 当前没有前端消费者。用量页面通过 REST 查询，不是 WebSocket 实时刷新。
- 教育资源实际受后端 `/api/education` 统一前缀和 `edu:resource:list` 权限约束，不是匿名静态资源。
