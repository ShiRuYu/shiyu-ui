...
- 后端业务路由：登录后调用 `/menu/all` 获取，由 Vben access 模块动态生成。

`import.meta.glob('../views/**/*.vue')` 生成页面组件注册表。后端菜单 `component` 示例 `/knowledge/spaces/index` 会规范化为 `../views/knowledge/spaces/index.vue`。
...
- 主布局和个人设置页在租户/角色切换后会重置动态路由、清空访问状态并触发 `/menu/all` 重载；菜单刷新逻辑符合后端上下文切换设计。
- 自动 Token 续期仍有后端过滤器契约风险：`/auth/refresh` 未被排除，而前端刷新请求只在 body 发送旧 Token。当前可确认的是续期失败后单次跳转登录，不能声称失效 Token 一定能够自动续期成功。
- 后端 `/ws/usage` 当前没有前端消费者。用量页面通过 REST 查询，不是 WebSocket 实时刷新。
- 教育资源实际受后端 `/edu` 统一前缀和 `edu:resource:list` 权限约束，不是匿名静态资源。
