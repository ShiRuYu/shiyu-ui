...
所有 41 个目录、页面和初始角色的逐条记录见 [页面路由与角色清单](参考/页面路由与角色清单.md)。
...
| 页面 | 路径 | 主要操作 |
| --- | --- | --- |
| Agent Studio | `/app-studio/agents` | 查询、新建、编辑、删除、发布和编排 Agent |
| Agent 编辑 | `/app-studio/agents/edit` | 编辑基础信息、编排节点和连线、校验并保存版本 |
| Prompt Studio | `/app-studio/prompts` | 管理模板、变量、预览和发布版本 |
| 意图路由 | `/app-studio/intents` | 维护意图、样例与路由策略 |
| Chat 工作区 | `/workspace/chat` | 发送消息、流式输出、编辑、重试和分支 |
| Agent 工作区 | `/workspace/agent` | 运行已发布 Agent App |
...
| 页面 | 路径 | 主要操作 |
| --- | --- | --- |
| 空间管理 | `/knowledge-center/spaces` | 新建空间、设置成员和默认空间 |
| 文档中心 | `/knowledge-center/documents` | 上传、查看版本、解析、索引、删除 |
| 图谱洞察 | `/knowledge-center/graph` | 选择中心知识点、展开关系、查看节点详情 |
| 检索实验室 | `/knowledge-center/search` | 选择空间、查询、调整 topK 并检查召回片段 |
| 知识评测 | `/knowledge-center/evaluations` | 维护评测用例并比较检索结果 |
