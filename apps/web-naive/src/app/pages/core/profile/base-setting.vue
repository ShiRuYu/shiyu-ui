...
import { getUserInfoApi } from '#/features/iam';
import { getTimezone, getTimezoneOptions, setTimezone } from '#/features/iam';
import { updateUser } from '#/features/iam';
...
async function handleUpdateProfile(values: Record<string, any>) {
  try {
    ...
    if (userStore.currentTenantId === null) {
      message.error('当前租户不可用，请重新登录');
      return;
    }
    await updateUser(Number(userId), {
      nickName: values.nickName,
      email: values.email,
      tenantId: userStore.currentTenantId,
    });
