...
import { deleteTextbook, getTextbookList } from '#/features/education/api';
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }) => {
          const result = await getTextbookList(page.currentPage, page.pageSize);
    ...
    <Grid :table-title="$t('education.textbook.list')">
      <template #toolbar-tools>
        <NButton
          type="primary"
          @click="onCreate"
          v-access:code="['edu:textbook:create']"
        >
          <Plus class="size-5" />
