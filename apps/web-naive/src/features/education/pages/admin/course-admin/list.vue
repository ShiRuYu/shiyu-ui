...
import { deleteCourse, getCourseList } from '#/features/education/api';
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }) => {
          const result = await getCourseList(page.currentPage, page.pageSize);
    ...
    <Grid :table-title="$t('education.course.list')">
      <template #toolbar-tools>
        <NButton
          type="primary"
          @click="onCreate"
          v-access:code="['edu:course:create']"
        >
          <Plus class="size-5" />
