...
import type { EducationCourseApi } from '#/features/education/api';

import { z } from '#/adapter/form';
import { getSubjectOptions } from '#/features/education/api';
import { getTextbookOptions } from '#/features/education/api';
  ...
  return [
    ...
    },
    {
      ...
      rules: z
        ...
        .min(
          1,
          $t('ui.formRules.required', [$t('education.course.subjectCode')]),
        ),
    },
    {
    ...
    },
    {
      component: 'Input',
      fieldName: 'coverUrl',
      label: $t('education.course.coverUrl'),
    },
    {
      component: 'Input',
      fieldName: 'description',
      label: $t('education.course.description'),
    },
    {
      component: 'InputNumber',
      fieldName: 'totalHours',
      label: $t('education.course.totalHours'),
    },
  ];
}

export function useColumns(
  onActionClick?: OnActionClickFn<EducationCourseApi.Course>,
): VxeTableGridColumns<EducationCourseApi.Course> {
  return [
    ...
    {
      field: 'totalHours',
      title: $t('education.course.totalHours'),
      width: 100,
    },
    { field: 'status', title: $t('system.user.status'), width: 80 },
    {
      ...
      cellRender: {
        attrs: {
          nameField: 'name',
          nameTitle: $t('education.course.name'),
