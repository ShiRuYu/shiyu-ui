...
import type { NodeTypeApi } from '#/features/agent/api';
...
import { getNodeTypes } from '#/features/agent/api';
import {
  activateVersion,
  archiveVersion,
  createVersion,
...
async function loadAgentOptions() {
  ...
  try {
    const opts = (await getAdminAgentListAll()) || [];
...
async function loadAgentDetail(id: number) {
  ...
  try {
    const detail = await getAdminAgentById(id);
    if (!detail) return;
...
async function handleCreateNewAgent() {
  ...
  try {
    const vo = await createAdminAgent({
      agentId: agentId.value,
...
async function handleSaveAgent() {
  ...
  try {
    await updateAdminAgent(agentDetailId.value, {
      agentId: agentId.value,
      name: agentName.value,
      description: agentDescription.value,
      status: agentStatus.value,
...
async function loadGraph() {
  ...
  try {
    ...
    const parsed = parseGraphConfig(detail.graphConfig);
    formNodes.value = parsed.formNodes;
    formEdges.value = parsed.formEdges;
    startNode.value = parsed.startNode;
    endNode.value = parsed.endNode;
    try {
