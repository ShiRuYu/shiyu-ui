...
async function createMedia(data: Omit<MediaApi.Media, 'id'>) {
  return requestClient.post('/api/record/media/create', data);
}

async function updateMedia(id: number, data: Partial<MediaApi.Media>) {
  return requestClient.post('/api/record/media/update', data, {
    params: { id },
  });
}

async function deleteMedia(id: number) {
  return requestClient.post('/api/record/media/delete', null, {
    params: { id },
  });
}
