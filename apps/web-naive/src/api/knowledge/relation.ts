...
export function getKnowledgePrerequisitesListApi(knowledgeId: number) {
  return requestClient.get<any[]>(`/api/v1/knowledge/${knowledgeId}/prerequisites-list`);
}

/** 获取后续知识点列表 */
export function getKnowledgeSubsequentListApi(knowledgeId: number) {
  return requestClient.get<any[]>(`/api/v1/knowledge/${knowledgeId}/subsequent-list`);
}
...
export function addKnowledgeRelationApi(data: {
  ...
  return requestClient.post('/api/v1/knowledge/relation', null, {
    params: { sourceId: data.sourceId, targetId: data.targetId, type: data.type, weight: data.weight ?? 1.0 },
  });
}
