...
import { ref } from 'vue';
import { NModal, NForm, NFormItem, NInput, NSelect, NButton, NSpace, NTag } from 'naive-ui';
import { createDocumentApi, updateDocumentApi } from '#/api/knowledge/document';
import { getKnowledgeListApi } from '#/api/knowledge';

const visible = ref(false);
const formData = ref<any>({ title: '', content: '', docType: 'ARTICLE', source: '', author: '', knowledgeIds: [] });
...
async function open(row?: any, defaultKnowledgeId?: number) {
  ...
  if (row) {
    ...
    formData.value = { title: '', content: '', docType: 'ARTICLE', source: '', author: '', knowledgeIds: defaultKnowledgeId ? [defaultKnowledgeId] : [] };
  }
  visible.value = true;
}
...
<template>
  <NModal v-model:show="visible" title="文档" :mask-closable="false" preset="card" style="width:700px">
    <NForm :model="formData" label-placement="left" label-width="80">
      ...
      <NFormItem label="类型">
        <NSelect v-model:value="formData.docType" :options="[
      ...
      <NFormItem label="关联知识点">
        <NSelect v-model:value="formData.knowledgeIds" :options="knowledgeOptions" multiple filterable placeholder="选择关联知识点" />
      </NFormItem>
      <NFormItem label="来源">
      ...
      <NFormItem label="内容" path="content" :rule="{ required: true, message: '请输入内容' }">
        <NInput v-model:value="formData.content" type="textarea" rows="8" placeholder="文档内容" />
      </NFormItem>
    </NForm>
    <template #footer>
      <NSpace justify="end">
