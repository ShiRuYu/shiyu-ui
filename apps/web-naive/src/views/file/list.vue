...
import { NCard, NDataTable, NProgress, NUpload, NUploadDragger, NIcon } from 'naive-ui';
...
function handleUpload({ file }: any) {
  ...
  upload_file({
    file,
    onProgress: (p) => { uploadProgress.value = p.percent; },
    onSuccess: (data, f) => {
      files.value.unshift({ name: f.name, size: f.size, time: new Date().toLocaleString(), url: data?.url || '-' });
...
<template>
  <Page :title="$t('page.file.title')">
    <NCard>
      <NUpload :default-upload="false" :multiple="false" @before-upload="handleUpload">
        <NUploadDragger>
      ...
      <NProgress v-if="uploading" class="mt-4" type="line" :percentage="uploadProgress" :show-indicator="true" />
    </NCard>

    <NCard class="mt-4" title="已上传文件">
      <NDataTable :columns="columns" :data="files" striped :row-key="(row: any) => row.name" />
      <div v-if="files.length === 0" class="py-10 text-center text-gray-400">暂无文件</div>
    </NCard>
  </Page>
</template>
