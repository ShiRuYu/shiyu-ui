...
import { createCourse, updateCourse } from '#/api/education/course';
import { $t } from '#/locales';
import { useSchema } from '../data';

const emit = defineEmits(['success']);
const formData = ref<EducationCourseApi.Course>();
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('education.course.name')]) : $t('ui.actionTitle.create', [$t('education.course.name')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id ? updateCourse(formData.value.id, data) : createCourse(data));
