...
function onEdit(row: EducationChapterApi.Chapter) { formModalApi.setData(row).open(); }
...
function onDelete(row: EducationChapterApi.Chapter) {
  ...
  deleteChapter(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.name])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationChapterApi.Chapter>) {
    ...
    <Grid :table-title="$t('education.chapter.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.chapter.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
