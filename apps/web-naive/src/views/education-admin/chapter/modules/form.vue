...
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('education.chapter.name')]) : $t('ui.actionTitle.create', [$t('education.chapter.name')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id ? updateChapter(formData.value.id, data) : createChapter(data));
