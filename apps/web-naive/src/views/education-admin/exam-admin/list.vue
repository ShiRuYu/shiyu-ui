...
function onEdit(row: EducationExamApi.Exam) { formModalApi.setData(row).open(); }
...
function onDelete(row: EducationExamApi.Exam) {
  ...
  deleteExam(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.name])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationExamApi.Exam>) {
    ...
    <Grid :table-title="$t('education.exam.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.exam.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
