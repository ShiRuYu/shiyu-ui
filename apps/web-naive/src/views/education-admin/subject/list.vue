...
function onEdit(row: EducationSubjectApi.Subject) { formModalApi.setData(row).open(); }
...
function onDelete(row: EducationSubjectApi.Subject) {
  ...
  deleteSubject(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.name])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationSubjectApi.Subject>) {
    ...
    <Grid :table-title="$t('education.subject.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.subject.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
