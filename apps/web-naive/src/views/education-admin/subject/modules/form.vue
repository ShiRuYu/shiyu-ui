...
import { createSubject, updateSubject } from '#/api/education/subject';
import { $t } from '#/locales';
import { useSchema } from '../data';

const emit = defineEmits(['success']);
const formData = ref<EducationSubjectApi.Subject>();
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('education.subject.name')]) : $t('ui.actionTitle.create', [$t('education.subject.name')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id ? updateSubject(formData.value.id, data) : createSubject(data));
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      const data = modalApi.getData<EducationSubjectApi.Subject>();
      formApi.resetForm();
