...
function onEdit(row: EducationTextbookApi.Textbook) { formModalApi.setData(row).open(); }
...
function onDelete(row: EducationTextbookApi.Textbook) {
  ...
  deleteTextbook(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.name])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationTextbookApi.Textbook>) {
    ...
    <Grid :table-title="$t('education.textbook.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.textbook.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
