...
import { createResource, updateResource } from '#/api/education/resource';
import { $t } from '#/locales';
import { useSchema } from '../data';

const emit = defineEmits(['success']);
const formData = ref<EducationResourceApi.Resource>();
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('education.resource.name')]) : $t('ui.actionTitle.create', [$t('education.resource.name')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id ? updateResource(formData.value.id, data) : createResource(data));
