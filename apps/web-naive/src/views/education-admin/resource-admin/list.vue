...
import { deleteResource, getResourceList } from '#/api/education/resource';
import { $t } from '#/locales';
import { useColumns, useGridFormSchema } from './data';
import Form from './modules/form.vue';

const [FormModal, formModalApi] = useVbenModal({ connectedComponent: Form, destroyOnClose: true });
function onEdit(row: EducationResourceApi.Resource) { formModalApi.setData(row).open(); }
function onCreate() { formModalApi.setData({}).open(); }
function onDelete(row: EducationResourceApi.Resource) {
  const h = message.loading($t('common.deleting'), { duration: 0 });
  deleteResource(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.name])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationResourceApi.Resource>) {
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('education.resource.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.resource.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
