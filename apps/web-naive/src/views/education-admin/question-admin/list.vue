...
import { deleteQuestion, getQuestionBySubjectGrade } from '#/api/education/question';
import { $t } from '#/locales';
import { useColumns, useGridFormSchema } from './data';
import Form from './modules/form.vue';

const [FormModal, formModalApi] = useVbenModal({ connectedComponent: Form, destroyOnClose: true });
function onEdit(row: EducationQuestionApi.Question) { formModalApi.setData(row).open(); }
...
function onDelete(row: EducationQuestionApi.Question) {
  ...
  deleteQuestion(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.code])); refreshGrid(); }).finally(() => h.destroy());
}
function onActionClick({ code, row }: OnActionClickParams<EducationQuestionApi.Question>) {
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async () => {
          const result = await getQuestionBySubjectGrade('', 0);
    ...
    <Grid :table-title="$t('education.question.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('education.question.name')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
