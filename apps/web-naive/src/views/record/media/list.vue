...
function onEdit(row: MediaApi.Media) { formModalApi.setData(row).open(); }
function onCreate() { formModalApi.setData({}).open(); }

function onDelete(row: MediaApi.Media) {
  const h = message.loading('正在删除...', { duration: 0 });
  deleteMedia(row.id).then(() => { message.success('删除成功'); refreshGrid(); }).finally(() => h.destroy());
}
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: { query: async (params, formValues) => {
        const search = Object.fromEntries(Object.entries(formValues || {}).filter(([, v]) => v !== '' && v !== null && v !== undefined));
        return await getMediaPage({ page: params.page?.currentPage || 1, pageSize: params.page?.pageSize || 10, ...search });
      }},
    },
  ...
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('record.media.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate"><Plus class="size-5" />{{ $t('ui.actionTitle.create', [$t('record.media.name')]) }}</NButton>
      </template>
    </Grid>
  </Page>
</template>
