...
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('record.records.name')]) : $t('ui.actionTitle.create', [$t('record.records.name')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try { await (formData.value?.id ? updateRecord({ ...formData.value, ...data }) : createRecord(data)); message.success($t('ui.actionMessage.operationSuccess')); modalApi.close(); emit('success'); }
      catch (error) { console.error(error); } finally { modalApi.lock(false); }
    }
  },
  async onOpenChange(isOpen) {
    if (isOpen) { const data = modalApi.getData<RecordsApi.Record>(); formApi.resetForm(); formData.value = data?.id ? data : undefined; await nextTick(); if (data?.id) formApi.setValues(data); }
  },
});
</script>
<template>
