...
function onEdit(row: RecordsApi.Record) { formModalApi.setData(row).open(); }
...
function onDelete(row: RecordsApi.Record) {
  ...
  deleteRecord(row.id).then(() => { message.success('删除成功'); refreshGrid(); }).finally(() => h.destroy());
}
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    ...
    proxyConfig: { ajax: { query: async (params, formValues) => {
      ...
      return await getRecordPage({ page: params.page?.currentPage || 1, pageSize: params.page?.pageSize || 10, ...search });
    }}},
  ...
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('record.records.list')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate"><Plus class="size-5" />{{ $t('ui.actionTitle.create', [$t('record.records.name')]) }}</NButton>
      </template>
    </Grid>
  </Page>
</template>
