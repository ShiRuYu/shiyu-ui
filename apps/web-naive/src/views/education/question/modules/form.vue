...
function open(row?: any) {
  ...
  if (row) {
    ...
    formData.value = { code: '', type: '', subjectCode: '', grade: null, difficulty: 2, abilityDimension: '', title: '', options: '', answer: '', analysis: '', source: '', tags: '', status: 1 };
  }
  visible.value = true;
  loadSubjectOptions();
}
...
<template>
  <NModal v-model:show="visible" title="题目" :mask-closable="false" preset="card" style="width:650px">
    <NForm :model="formData" label-placement="left" label-width="100">
      <NFormItem label="编码" path="code">
        <NInput v-model:value="formData.code" placeholder="题目编码" :disabled="isEdit" />
      </NFormItem>
      <NFormItem label="类型" :rule="{ required: true, message: '请选择类型' }">
        <NSelect v-model:value="formData.type" :options="typeOptions" placeholder="请选择题型" />
      </NFormItem>
      <NFormItem label="学科">
      ...
      <NFormItem label="难度">
        <NSelect v-model:value="formData.difficulty" :options="difficultyOptions" placeholder="请选择难度" />
      </NFormItem>
      <NFormItem label="能力维度">
        <NInput v-model:value="formData.abilityDimension" placeholder="如 remember/understand/apply" />
      </NFormItem>
      <NFormItem label="标题" :rule="{ required: true, message: '请输入标题' }">
        <NInput v-model:value="formData.title" type="textarea" rows="2" placeholder="题目内容" />
      </NFormItem>
      <NFormItem label="选项">
        <NInput v-model:value="formData.options" type="textarea" rows="3" placeholder='JSON数组，如 ["A.选项1","B.选项2"]' />
      </NFormItem>
      <NFormItem label="答案">
        <NInput v-model:value="formData.answer" placeholder="正确答案" />
      </NFormItem>
      <NFormItem label="解析">
        <NInput v-model:value="formData.analysis" type="textarea" rows="3" placeholder="答案解析" />
      </NFormItem>
      <NFormItem label="来源">
