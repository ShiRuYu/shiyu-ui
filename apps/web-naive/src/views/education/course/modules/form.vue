...
function open(row?: any) {
  ...
  if (row) {
    ...
    formData.value = { name: '', description: '', subjectCode: '', grade: null, textbookId: null, teacherId: null, coverUrl: '', totalHours: 0, status: 1 };
  }
  visible.value = true;
  loadSubjectOptions();
}
...
<template>
  <NModal v-model:show="visible" title="课程" :mask-closable="false" preset="card" style="width:600px">
    <NForm :model="formData" label-placement="left" label-width="80">
      ...
      <NFormItem label="描述">
        <NInput v-model:value="formData.description" type="textarea" rows="3" placeholder="课程描述" />
      </NFormItem>
      <NFormItem label="学科">
      ...
      <NFormItem label="教材ID">
        <NInputNumber v-model:value="formData.textbookId" :min="0" placeholder="关联教材ID" style="width:100%" />
      </NFormItem>
      ...
      <NFormItem label="总课时">
        <NInputNumber v-model:value="formData.totalHours" :min="0" style="width:100%" />
      </NFormItem>
      <NFormItem label="状态">
