...
import { NButton, NDataTable, NSpace, NCard, NInput, NInputNumber, NSelect } from 'naive-ui';
import { getTextbookListApi, deleteTextbookApi, getTextbookBySubjectAndGradeApi } from '#/api/education/textbook';
...
async function fetchData() {
  ...
  try {
    if (filterSubjectCode.value && filterGrade.value) {
      const res = await getTextbookBySubjectAndGradeApi(filterSubjectCode.value, filterGrade.value);
      tableData.value = res || [];
    } else {
...
<template>
  <NCard title="教材管理" :bordered="false" class="h-full">
    <template #header-extra>
      <NSpace>
        <NInput v-model:value="searchName" placeholder="搜索教材..." clearable style="width:160px" />
        <NSelect v-model:value="filterSubjectCode" :options="subjectOptions" placeholder="学科" clearable style="width:120px" />
        <NSelect v-model:value="filterGrade" :options="gradeOptions" placeholder="年级" clearable style="width:120px" />
        ...
        <NButton type="primary" @click="() => formModalRef?.open()">新增教材</NButton>
      </NSpace>
    </template>
    <NDataTable
      :columns="columns"
