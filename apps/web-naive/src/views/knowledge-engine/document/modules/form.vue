...
const getTitle = computed(() => formData.value?.id ? $t('ui.actionTitle.edit', [$t('knowledge.documentTitle')]) : $t('ui.actionTitle.create', [$t('knowledge.documentTitle')]));
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id ? updateDocumentApi(formData.value.id, data) : createDocumentApi(data));
