...
import { deleteDocumentApi, searchDocumentsApi } from '#/api/knowledge/document';
...
function onDelete(row: any) {
  ...
  deleteDocumentApi(row.id).then(() => { message.success($t('ui.actionMessage.deleteSuccess', [row.title])); refreshGrid(); }).finally(() => h.destroy());
}
    ...
    <Grid :table-title="$t('knowledge.document')">
      <template #toolbar-tools>
        <NButton type="primary" @click="onCreate">
          <Plus class="size-5" /> {{ $t('ui.actionTitle.create', [$t('knowledge.documentTitle')]) }}
        </NButton>
      </template>
    </Grid>
  </Page>
