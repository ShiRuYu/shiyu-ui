...
<template>
  <Page :title="$t('page.aiTutor.chat')">
    <NCard class="flex flex-col" style="min-height: 500px">
      <div class="flex-1 space-y-4 overflow-y-auto p-4" style="max-height: 400px">
        ...
        <div v-if="messages.length === 0" class="py-10 text-center text-gray-400">
          开始与AI对话
        </div>
      </div>
      ...
      <NSpace class="mt-4">
        ...
        />
        <NButton type="primary" :loading="loading" @click="sendMessage">{{ $t('aiTutor.send') }}</NButton>
      </NSpace>
    </NCard>
  </Page>
</template>
