...
// Domain facades own the complete `/api/{domain}` path. Keep the client
// origin-only so the `/api` prefix is not duplicated in dev or production.
const requestBaseURL = apiURL === '/api' ? '' : apiURL;
...
function createRequestClient(baseURL: string, options?: RequestClientOptions) {
  ...
  async function doRefreshToken() {
    const accessStore = useAccessStore();
    const resp = await baseRequestClient.post('/api/iam/auth/refresh', {
