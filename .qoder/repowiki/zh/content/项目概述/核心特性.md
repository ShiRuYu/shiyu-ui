...
本项目不再限制于特定 UI 框架，支持以下主流 UI 库，通过适配器模式达到统一接口：

| UI 库          | 应用             | 说明           |
| -------------- | ---------------- | -------------- |
| Naive UI       | `apps/web-naive` | 当前项目主应用 |
| Ant Design Vue | `playground`     | 参考示例应用   |
| Element Plus   | 可扩展           | 官方支持       |
| TDesign        | 可扩展           | 官方支持       |

适配器层位于 `src/adapter/`，包括：

- `adapter/component/index.ts` — 组件映射与全局注册
- `adapter/form.ts` — 表单适配器配置（`setupVbenForm`）
- `adapter/vxe-table.ts` — vxe-table 表格适配器
