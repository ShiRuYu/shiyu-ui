...
- 权限相关：用户状态管理、重置密码
- 数据模型与字段约束：用户名、昵称、邮箱、手机号、部门、状态、备注、创建时间等
...
- 用户对象关键字段
  ...
  - status：数值，0/1，必填，默认启用
  - remark：字符串，可选，最大长度200
  - createTime：字符串，只读
  - avatar、roleIds 等扩展字段存在但未在表单/列表中强制使用

- 字段约束与校验
  - 必填项：username、nickname
  - 邮箱：可选但需合法
  - 手机号：可选但需合法
  - 备注：最多200字符
  - 状态：仅允许0/1
...
- 业务规则
  ...
  - 密码安全：重置密码接口用于设置新密码，具体强度策略由后端决定
  - 状态管理：通过状态字段控制启停
...
- 获取用户列表（分页）
  ...
  - 示例
    - 请求：GET /user?pageNo=1&pageSize=10&status=1
...
- 用户角色分配
  - 用户模型包含 roleIds 字段，但前端未提供角色选择控件
...
- [apps/web-naive/src/views/system/user/data.ts:125-191](file://apps/web-naive/src/views/system/user/data.ts#L125-L191)

### 请求与响应示例

- 获取用户列表
  - 请求
    - GET /user?pageNo=1&pageSize=10&status=1
  - 成功响应
...
- 创建用户
  - 请求
    - POST /user
    - Body: { username, nickname, email, phone, deptId, status, remark }
  - 成功响应
...
```mermaid
flowchart TD
Start(["开始"]) --> BuildQuery["构造查询参数<br/>pageNo/pageSize/过滤条件"]
...
```mermaid
flowchart TD
Open(["打开表单"]) --> Validate["表单校验"]
Validate --> Valid{"校验通过？"}
Valid --> |否| ShowError["提示错误"]
Valid --> |是| Submit["调用创建/更新接口"]
Submit --> Success{"请求成功？"}
...
- [apps/web-naive/src/views/system/user/modules/form.vue:37-59](file://apps/web-naive/src/views/system/user/modules/form.vue#L37-L59)
- [apps/web-naive/src/views/system/user/data.ts:47-120](file://apps/web-naive/src/views/system/user/data.ts#L47-L120)
...
LOCALE["locales/zh-CN/system.json"] --> LIST
LOCALE --> FORM
...
本文档梳理了用户管理 API 的接口规范、数据模型、权限与高级功能、错误处理与性能建议，并提供了关键流程的可视化说明。建议后续完善：
...
- 分页查询用户列表
  - 在列表页使用表格代理查询，传入 pageNo、pageSize 与过滤条件
- 新增用户
  - 打开表单弹窗，填写必填字段，点击确认提交
- 编辑用户
