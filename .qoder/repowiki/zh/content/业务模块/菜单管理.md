...
- 更新状态字段数据类型：从数字值(0|1)改为字符串值('0'|'1')
- 新增菜单类型专用组件：针对不同菜单类型提供专门的表单组件
- 增强表单schema：完善字段验证和依赖关系
- 优化菜单渲染：改进图标显示和徽标组件集成
- 扩展元信息配置：增加更多显示控制选项
...
**更新** 本版本重点更新了状态字段的数据类型处理、菜单类型的专用组件支持以及表单schema的增强配置。
...
- [apps/web-naive/src/views/system/menu/data.ts:1-345](file://apps/web-naive/src/views/system/menu/data.ts#L1-L345)
...
- [packages/utils/src/helpers/generate-routes-backend.ts:1-109](file://packages/utils/src/helpers/generate-routes-backend.ts#L1-L109)
...
**更新** 状态字段现使用字符串值('0'|'1')而非数字值，表单schema针对不同菜单类型提供专用组件支持。

**章节来源**

- [apps/web-naive/src/views/system/menu/list.vue:1-155](file://apps/web-naive/src/views/system/menu/list.vue#L1-L155)
- [apps/web-naive/src/views/system/menu/modules/form.vue:1-94](file://apps/web-naive/src/views/system/menu/modules/form.vue#L1-L94)
- [apps/web-naive/src/views/system/menu/data.ts:46-251](file://apps/web-naive/src/views/system/menu/data.ts#L46-L251)
- [apps/web-naive/src/api/system/menu.ts:25-104](file://apps/web-naive/src/api/system/menu.ts#L25-L104)
- [apps/web-naive/src/router/guard.ts:47-126](file://apps/web-naive/src/router/guard.ts#L47-L126)
...
- [apps/web-naive/src/api/system/menu.ts:146-181](file://apps/web-naive/src/api/system/menu.ts#L146-L181)
- [apps/web-naive/src/router/guard.ts:105-126](file://apps/web-naive/src/router/guard.ts#L105-L126)
- [apps/web-naive/src/router/access.ts:16-38](file://apps/web-naive/src/router/access.ts#L16-L38)
- [packages/utils/src/helpers/generate-routes-backend.ts:22-53](file://packages/utils/src/helpers/generate-routes-backend.ts#L22-L53)
...
- **更新** 状态字段现使用字符串值('0'|'1')而非数字值(0|1)，确保与表单schema一致
...
- **更新** 状态列使用 CellTag 组件渲染字符串状态值('0'|'1')
...
Columns --> Status["状态列: '0'|'1' 字符串值"]
Status --> Actions["操作: 新增下级/编辑/删除"]
Actions --> End(["完成"])
```
...
- [apps/web-naive/src/views/system/menu/data.ts:257-344](file://apps/web-naive/src/views/system/menu/data.ts#L257-L344)
...
- **更新** 状态字段使用字符串值('0'|'1')的 Switch 组件，提供更直观的状态选择

```mermaid
flowchart TD
Open(["打开表单"]) --> LoadData["读取传入数据<br/>处理 pid=0/null"]
LoadData --> Schema["加载表单schema<br/>类型驱动的字段配置"]
Schema --> Validate["按类型校验字段<br/>联动显隐与验证"]
Validate --> Submit["提交: 创建/更新<br/>状态: '0'|'1' 字符串"]
...
- **更新** 增加徽标组件(MenuBadge)支持，提供 dot 和 normal 两种徽标类型
...
- **更新** 状态显示异常：检查状态字段是否为字符串值('0'|'1')而非数字值(0|1)
...
菜单管理模块以清晰的数据模型与表单配置为核心，配合树形表格与动态路由生成，实现了完整的菜单生命周期管理。通过权限生成器与路由守卫，系统在运行时动态注入可访问菜单与路由，满足不同权限场景需求。**更新后的版本**增强了状态字段处理、菜单类型专用组件支持和表单schema配置，建议在现有基础上扩展批量操作、排序与导入导出能力，并结合缓存策略进一步优化性能。
