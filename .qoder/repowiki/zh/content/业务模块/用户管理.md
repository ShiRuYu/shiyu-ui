...
DEPTAPI["部门API<br/>api/system/dept.ts"]
...
- [apps/web-naive/src/api/system/dept.ts:16-31](file://apps/web-naive/src/api/system/dept.ts#L16-L31)
...
- [apps/web-naive/src/views/system/user/list.vue:1-177](file://apps/web-naive/src/views/system/user/list.vue#L1-L177)
...
- **新增功能**
  - 密码字段处理：新增时必须填写，编辑时可选
  - 字段映射：处理 nickname 与 nickName 的双向映射
  - 类型转换：确保 status 和 sex 为字符串类型
...
- **表单schema**：用户名、昵称、邮箱、手机、所属部门、性别、角色、状态、备注，并内置多项校验规则
- **校验策略**：必填、邮箱格式、手机号格式、密码长度、最大长度等
- **新增功能**：性别选择、角色多选、条件密码验证
...
- **角色列表**：新增 getRoleList 接口用于用户表单中的角色选择
...
- [apps/web-naive/srcapi/system/role.ts:1-59](file://apps/web-naive/src/api/system/role.ts#L1-L59)

### 部门API（api/system/dept.ts）

- **部门列表**：提供获取部门列表的能力
- **部门CRUD**：提供创建、更新、删除部门接口
- **表格适配**：提供 getDeptListForGrid 用于表格组件的数据格式化
...
LD --> RAPI["api/system/role.ts"]
LD --> DEPTAPI["api/system/dept.ts"]
```
...
- **新增用户失败**
  ...
  - 检查表单验证规则与字段映射
- **角色分配无效**
  - 确认角色多选组件是否正确绑定 roleIds 字段
  - 检查 getRoleList 接口返回的数据格式
- **登录态失效**
  - 检查自动刷新令牌流程与重新认证逻辑
  - 确认访问码与菜单生成是否正常
...
用户管理模块以清晰的分层设计实现了用户列表、表单编辑、创建与删除、密码重置等核心功能，并通过统一请求客户端与认证体系保障了安全性与一致性。当前版本已增强用户界面功能，包括新增密码字段、性别选择、角色分配和改进的表单验证。这些变更提升了用户体验和数据完整性。建议在后续迭代中补充批量操作、数据导入导出与更丰富的搜索过滤能力。
...
- **字段说明**（来源于用户API模型与表单配置）
  - id：用户唯一标识
  - username：用户名
  - nickname/nickName：昵称（支持双向映射）
  - email：邮箱（可选，需符合邮箱格式）
  - phone：手机号（可选，需符合手机号格式）
  - deptId：所属部门ID（通过 ApiSelect 选择）
  - sex：性别（0=男，1=女，2=未知，默认未知）
  - roleIds：角色ID集合（用于角色分配）
  - status：状态（启用/禁用，默认启用）
  - remark：备注（最多200字符）
  - createTime：创建时间
  - password：密码（新增时必填，编辑时可选）
...
- [apps/web-naive/src/views/system/user/data.ts:54-166](file://apps/web-naive/src/views/system/user/data.ts#L54-L166)

### 权限与角色关联机制

- **现状**：用户模型包含 roleIds 字段，角色列表可通过 getRoleList 获取
- **实现**：用户表单中使用 ApiSelect 组件，支持多选角色
...
- [apps/web-naive/src/views/system/user/data.ts:124-135](file://apps/web-naive/src/views/system/user/data.ts#L124-L135)
...
- [apps/web-naive/src/views/system/user/data.ts:137-149](file://apps/web-naive/src/views/system/user/data.ts#L137-L149)
- [apps/web-naive/src/views/system/user/data.ts:220-228](file://apps/web-naive/src/views/system/user/data.ts#L220-L228)

### 性别管理

- **性别字段**：sex（"0"|"1"|"2"），通过 RadioGroup 控件进行选择
- **选项**：0=男，1=女，2=未知（默认）
- **展示**：表格列使用国际化文本显示性别

**章节来源**

- [apps/web-naive/src/views/system/user/data.ts:97-110](file://apps/web-naive/src/views/system/user/data.ts#L97-L110)
- [apps/web-naive/src/views/system/user/data.ts:202-214](file://apps/web-naive/src/views/system/user/data.ts#L202-L214)
...
- [apps/web-naive/src/views/system/user/data.ts:15-43](file://apps/web-naive/src/views/system/user/data.ts#L15-L43)
...
- **字段映射**：注意 nickname 与 nickName 的双向映射，确保前后端数据一致性
