...
每个表单项均为一个 Schema 对象，常用属性如下：

| 属性 | 描述 | 类型 |
| --- | --- | --- |
| `component` | 组件类型，需已在适配器中注册 | `ComponentType` |
| `fieldName` | 字段名 | `string` |
| `label` | 界面显示的标签 | `string` |
| `componentProps` | 对应组件的参数 | `object` |
| `defaultValue` | 默认值 | `any` |
| `rules` | 校验规则（支持 Zod / 内置规则） | `ZodType\|string` |
| `required` | 是否必填（快捷配置） | `boolean` |
| `disabled` | 是否禁用 | `boolean` |
| `hidden` | 是否隐藏 | `boolean` |
| `suffix` | 尾缀插槽 | `()=>VNode` |
| `renderComponentContent` | 自定义组件内容插槽 | `()=>object` |
| `dependencies` | 字段联动配置 | `object` |
| `wrapperClass` | 单个字段的栅格占位 | `string` |
| `formItemClass` | 表单项 class | `string` |
| `labelClass` | 标签 class | `string` |
...
| `setState(state)`             | 动态更新表单配置 |
| `getState()`                  | 获取表单当前状态 |

### 表单适配器配置（setupVbenForm）

在 `src/adapter/form.ts` 中配置，主要参数：

```ts
setupVbenForm<ComponentType>({
  config: {
    // 组件库默认 v-model 绑定名
    // Naive UI: 'value'（默认）；Ant Design Vue: 'value'
    baseModelPropName: 'value',
    // 重置时空值（与组件库一致）。Ant Design Vue 用 null，Naive UI 用 undefined
...
| 组件类型        | 对应组件               | v-model 属性 |
| --------------- | ---------------------- | ------------ |
| `Input`         | NInput                 | `value`      |
| `InputNumber`   | NInputNumber           | `value`      |
| `InputPassword` | NInput (type=password) | `value`      |
| `Select`        | NSelect                | `value`      |
| `TreeSelect`    | NTreeSelect            | `value`      |
| `Checkbox`      | NCheckbox              | `checked`    |
| `CheckboxGroup` | NCheckboxGroup         | `value`      |
| `Radio`         | NRadio                 | `checked`    |
| `RadioGroup`    | NRadioGroup            | `value`      |
| `DatePicker`    | NDatePicker            | `value`      |
| `Switch`        | NSwitch                | `value`      |
| `TimePicker`    | NTimePicker            | `value`      |
| `Upload`        | NUpload                | `fileList`   |
| `Textarea`      | NInput (type=textarea) | `value`      |
